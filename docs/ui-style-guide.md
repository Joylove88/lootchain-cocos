...
稀有度配色（2026-07 统一，全端一致；仅客户端渲染色，数据库不存稀有度颜色）：

- R = 蓝 `#5D97FF`（`rgba(93,151,255)`）
- SR = 紫 `#C86FFF`（`rgba(200,111,255)`）
- SSR = 橙 `#FFA836`（`rgba(255,168,54)`）
- UR = 炽红 `#FF5430`（`rgba(255,84,48)`）
- 未知/无 = 灰 `#605B58`（`rgba(96,91,88)`）

以各渲染器内 `resolveRarityColor()` 为唯一真值（英雄名册/详情/布阵右栏/战斗卡牌/大厅挂机等共用同一映射），改色需同步全部 `resolveRarityColor`。
