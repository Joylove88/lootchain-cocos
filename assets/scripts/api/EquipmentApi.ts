...
export class EquipmentApi {
  ...
  list(): Promise<EquipmentItemVO[]> {
    return this.http.get<EquipmentItemVO[]>('/api/player/equipment');
  }

  equip(equipmentId: number, heroId: number): Promise<EquipmentItemVO> {
    return this.http.post<EquipmentItemVO>('/api/player/equipment/equip', { equipmentId, heroId });
  }

  unequip(equipmentId: number): Promise<EquipmentItemVO> {
    return this.http.post<EquipmentItemVO>('/api/player/equipment/unequip', { equipmentId });
  }

  // 合成(2.0 P2):3 件同部位同稀有度未穿戴 → 有概率合成上一稀有度;失败返还同档 1 件。
  fuse(equipmentIds: number[], useLuckStone: boolean): Promise<EquipFuseResultVO> {
    return this.http.post<EquipFuseResultVO>('/api/player/equipment/fuse', { equipmentIds, useLuckStone });
  }
  ...
  enhance(equipmentId: number, useBlessStone: boolean, useGuardRune: boolean): Promise<EquipEnhanceResultVO> {
    return this.http.post<EquipEnhanceResultVO>('/api/player/equipment/enhance', { equipmentId, useBlessStone, useGuardRune });
  }

  // 分解(2.0 P3):未穿戴装备换强化石。
  decompose(equipmentIds: number[]): Promise<EquipDecomposeResultVO> {
    return this.http.post<EquipDecomposeResultVO>('/api/player/equipment/decompose', { equipmentIds });
  }

  // 词条洗练(2.0 P4):洗练石×1+金币(紫500/橙2000/红5000),整件重roll全部词条。
  reroll(equipmentId: number): Promise<EquipRerollResultVO> {
    return this.http.post<EquipRerollResultVO>('/api/player/equipment/reroll', { equipmentId });
  }
  ...
  gemSocket(equipmentId: number, slotIndex: number, gemCode: string): Promise<EquipmentItemVO> {
    return this.http.post<EquipmentItemVO>('/api/player/equipment/gem/socket', { equipmentId, slotIndex, gemCode });
  }

  // 宝石拆卸(P5):金币费 100×2^(阶-1),宝石退回背包。
  gemUnsocket(equipmentId: number, slotIndex: number): Promise<EquipmentItemVO> {
    return this.http.post<EquipmentItemVO>('/api/player/equipment/gem/unsocket', { equipmentId, slotIndex });
  }
}
